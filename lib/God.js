...
God.executeApp = function executeApp(env, cb) {
  ...
  if (env_copy.exec_mode === 'cluster_mode') {
    ...
    God.nodeApp(env_copy, function nodeApp(err, clu) {
      ...
      if (clu.pm2_env.wait_app) {
        var listener = function (packet) {
          if (packet.process.name === clu.pm2_env.name && packet.process.pm_id === clu.pm2_env.pm_id) {
            God.bus.removeListener('ready', listener)
            readyCb()
          }
        }
        God.bus.on('ready', listener);
      }
      else
        clu.once('online', readyCb);
      return false;
    });
  }
  else {
    ...
    God.forkMode(env_copy, function forkMode(err, clu) {
      ...
      if (clu.pm2_env.wait_app) {
        var listener = function (packet) {
          if (packet.process.name === proc.pm2_env.name && packet.process.pm_id === proc.pm2_env.pm_id) {
            God.bus.removeListener('ready', listener)
            readyCb()
          }
        }
